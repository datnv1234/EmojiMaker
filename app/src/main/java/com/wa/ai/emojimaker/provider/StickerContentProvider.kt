package com.wa.ai.emojimaker.provider
...
import com.wa.ai.emojimaker.common.sharedprefs.initSharedPrefs
import com.wa.ai.emojimaker.data.model.StickerPackView
import com.wa.ai.emojimaker.utils.extention.getLastBitFromUrl
import java.io.File
    ...
    private fun fetchFile(uri: Uri, fileName: String, identifier: String): AssetFileDescriptor? {
        return try {
            val file: File = if (fileName.endsWith(".png")) {
                File(
                    context!!.filesDir.toString() + "/" + "stickers_asset" + "/" + "1" + "/try/",
                    fileName
                )
            } else {
                File(
                    context!!.filesDir.toString() + "/" + "stickers_asset" + "/" + "1" + "/",
                    fileName
                )
            }
            if (!file.exists()) {
    ...
    private fun getStickersForAStickerPack(uri: Uri): Cursor {
        ...
        for (stickerPack in getStickerPackList()) {
            if (identifier == stickerPack.identifier.toString()) {
                for (sticker in stickerPack.stickers) {
                    cursor.addRow(
                        sticker.imageFile?.let {
                            arrayOf<Any>(
                                it.getLastBitFromUrl(),
